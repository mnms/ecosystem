...
# CPU
$HOME/sbin/distcmd.sh top -b -n1 | grep "id\|fb"| awk '{if(NF == 1) print $1;else print 100-$8}' | xargs -n2 | grep fb > $RES_DIR/server-stat-cpu.dat
...
# Memory
$HOME/sbin/distcmd.sh free | awk '{if($1 == "Mem:") print "," $2 "," $3 ","  $7;else if(NF == 1) print $1}' | xargs -n2 > $RES_DIR/server-stat-memory.dat
...
# Disk
$HOME/sbin/distcmd.sh df | grep "fb\|\/DATA" | awk '{if(NF == 1) node=$1; else  print node ","  $3 "," $4 "," $5 "," $6}' > $RES_DIR/server-stat-disk.dat
...
mysql -e "truncate ${DB}.distribution" -u$DBID -p$PW -h$HOST
...
mysql -e "insert into ${DB}.configs (cluster, feature, value, description)  values ('${cluster_no}', '${key}', '${value}', '${desc}')" -u$DBID -p$PW -h$HOST
...
# Remove old data
mysql -e "DELETE FROM ${DB}.server_cpu WHERE event_time < DATE_SUB(NOW(), INTERVAL $KEEPDAYS DAY)" -u$DBID -p$PW -h$HOST
mysql -e "DELETE FROM ${DB}.server_memory WHERE event_time < DATE_SUB(NOW(), INTERVAL $KEEPDAYS DAY)" -u$DBID -p$PW -h$HOST
mysql -e "DELETE FROM ${DB}.server_disk WHERE event_time < DATE_SUB(NOW(), INTERVAL $KEEPDAYS DAY)" -u$DBID -p$PW -h$HOST
mysql -e "DELETE FROM ${DB}.used_memory WHERE event_time < DATE_SUB(NOW(), INTERVAL $KEEPDAYS DAY)" -u$DBID -p$PW -h$HOST
mysql -e "DELETE FROM ${DB}.datakeys WHERE event_time < DATE_SUB(NOW(), INTERVAL $KEEPDAYS DAY)" -u$DBID -p$PW -h$HOST
mysql -e "DELETE FROM ${DB}.partitions WHERE event_time < DATE_SUB(NOW(), INTERVAL $KEEPDAYS DAY)" -u$DBID -p$PW -h$HOST
mysql -e "DELETE FROM ${DB}.evictions WHERE event_time < DATE_SUB(NOW(), INTERVAL $KEEPDAYS DAY)" -u$DBID -p$PW -h$HOST
mysql -e "DELETE FROM ${DB}.storages WHERE event_time < DATE_SUB(NOW(), INTERVAL $KEEPDAYS DAY)" -u$DBID -p$PW -h$HOST
